...
| Cancelar cita | `POST /appointments/{id}/cancel` | USER |
| Solicitar reprogramación | `POST /appointments/{id}/reschedule-requests` | USER |
...
`POST /api/v1/appointments/{id}/cancel` permite a USER cancelar únicamente una cita propia futura cuyo estado no sea terminal. Cambia el estado a `CANCELLED`, libera todos sus slots y registra historial con fuente `USER`. Citas ajenas o no elegibles responden `404` o `409` sin modificar datos.

`POST /api/v1/appointments/{id}/reschedule-requests` recibe `date`, `startTime` y `reason` opcional. Solo acepta una cita propia futura `APPROVED`, conserva profesional/especialidad y crea una solicitud `PENDING`; la nueva franja se retiene mediante `professional_slots.reschedule_request_id`, mientras la cita y sus slots originales permanecen intactos. La decisión ADMIN pertenece a HU-028.
